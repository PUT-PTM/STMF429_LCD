...
public:
	...
	void setklik()
	{
		if (klik==false) klik = true;
		else if (klik == true) klik = false;
	}
...
private:
	int kolor; //0 - czerwony, 1 - zielony, 2 - niebieski
	int numerpola; //ostatecznie nieuzyte ze wzgledu na wykorzystanie tablic
	int ostatni=0; //indeks ostatniego uzywanego pionka (uzywany tylko w przycisku pom)
	bool klik = false; //czy bylo juz klikniecie czy nie w ruchu tego gracza
	bool wykonano = false; //czy wykonano ruch czy cofnac do stanu z przed klikniecia
	bool krolowa = false;
};
